...
def resolve(text):
    ...
    # ORIGIN: club immediately preceded by "from".
    for start, canon, key in clubs:
        if _ORIGIN_PREP.search(tn[:start]):
            from_club, from_key = canon, key
            break
    # ORIGIN fallback: a raw proper-noun after "from" not in our lexicon.
    if from_club is None:
        m = _RAW_ORIGIN.search(text or "")
        if m:
