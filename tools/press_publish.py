#!/usr/bin/env python3
"""Publish exactly one verified FPL press roundup per FPL gameweek."""
...
def _next_event(fpl_data: dict[str, Any], now: datetime) -> Optional[dict[str, Any]]:
    ...
    for event in fpl_data.get("events", []) or []:
        try:
            deadline = datetime.fromisoformat(str(event.get("deadline_time") or "").replace("Z", "+00:00"))
...
def _article_item(raw: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "summary": str(raw.get("text") or raw.get("title") or ""),
        "text": str(raw.get("text") or raw.get("title") or ""),
        "source_url": raw.get("link") or "https://fantasy.premierleague.com/api/bootstrap-static/",
        "publisher_url": raw.get("publisher_url") or "https://fantasy.premierleague.com/api/bootstrap-static/",
        "publisher_name": raw.get("publisher_name") or "Fantasy Premier League",
        "source_id": FPL_SOURCE_ID,
        "source_hint": FPL_SOURCE_ID,
...
def _candidate(raw: dict[str, Any], runtime: VerificationRuntime) -> Optional[tuple[int, str, dict[str, Any], dict[str, Any]]]:
    item = _article_item(raw)
    try:
        document = runtime.documents.from_item(item)
    except Exception:
        return None
    if not document.source.verified or document.source.profile_id != FPL_SOURCE_ID:
        return None

    item = enrich_official_item(item, runtime)
    if not item.get("full_text") or not is_fpl_url(item.get("source_url")):
        return None
    item.setdefault("metadata", {})["fpl_press_roundup"] = True
...
def _already_posted(lock: dict[str, Any], event_id: str) -> bool:
    return str(event_id) in {str(key) for key in (lock.get("posted_events") or {}).keys()}


def _mark_posted(lock: dict[str, Any], *, event: dict[str, Any], fingerprint: str, source_url: Optional[str]) -> None:
...
def _status(**values: Any) -> None:
    _write_json(RUN_STATUS_PATH, {"run_type": "press_conference_roundup", "updated_at": utcnow().isoformat(), **values})
...
async def run() -> int:
    ...
    if not in_window:
        _status(run_exit="outside_publication_window", posted_count=0, event_id=event["event_id"], window_reason=reason)
        return 0

    lock = _load_json(LOCK_PATH, {"schema_version": 1, "posted_events": {}})
    if _already_posted(lock, event["event_id"]):
        _status(run_exit="gameweek_already_posted", posted_count=0, event_id=event["event_id"])
    ...
    if str(collection.get("event_id") or "") != str(event["event_id"]):
        _status(run_exit="collection_not_ready", posted_count=0, event_id=event["event_id"])
        return 0
    ...
    try:
        ...
        rebuild_allowed = os.getenv(rollout["database_rebuild_environment_variable"], "") == rollout["database_rebuild_required_value"]
        if runtime.database_was_empty and not rebuild_allowed:
            _status(run_exit="verification_database_empty", posted_count=0)
            return 0

        candidates = [candidate for raw in collection.get("items", []) or [] if (candidate := _candidate(raw, runtime)) is not None]
        if not candidates:
            _status(run_exit="no_parseable_official_fpl_roundup", posted_count=0, collected_count=len(collection.get("items", []) or []))
            return 0

        _count, _published, source_item, story = max(candidates, key=lambda row: (row[0], row[1]))
        decision = runtime.verify_observations([{"document": bot._v2_document_item(source_item), "legacy_story": dict(story)}])
        validation = validate_official_press_conference(decision, runtime.sources)
        if not decision.may_publish or not validation.ok:
            _status(run_exit="roundup_not_publishable", posted_count=0, decision=decision.decision.value, reasons=decision.reasons[:10], press_gate=validation.reason)
            return 0

        if runtime.repository.has_publication_fingerprint(decision.fingerprint):
            _mark_posted(lock, event=event, fingerprint=decision.fingerprint, source_url=decision.source_url)
            _status(run_exit="already_published_fingerprint", posted_count=0, fingerprint=decision.fingerprint)
            return 0
        ...
        if bot.in_cooldown(data):
            _status(run_exit="x_cooldown_active", posted_count=0, cooldown_until=data.get("cooldown_until"))
            return 0
        ...
        if hour_cap is not None and bot._recent_post_count(data, 3600) >= hour_cap:
        ...
        try:
            ...
            bot.save_data(data)
            _status(run_exit="post_not_completed", posted_count=0, posting_failures=[{"error_type": type(exc).__name__}])
            print(f"[PRESS] X delivery failed: {type(exc).__name__}")
            return 1
        finally:
            await client.http.aclose()
        ledger_has_post = runtime.repository.has_publication_fingerprint(decision.fingerprint)
        if posted or ledger_has_post:
            _mark_posted(lock, event=event, fingerprint=decision.fingerprint, source_url=decision.source_url)
            _status(run_exit="posted" if posted else "x_duplicate_recorded", posted_count=1 if posted else 0, event_id=event["event_id"], event_name=event["event_name"], deadline=event["deadline"].isoformat(), window_start=window_start.isoformat() if window_start else None, fingerprint=decision.fingerprint, roundup_count=len(decision.verified_facts.get("roundup") or []), source_url=decision.source_url)
            print(f"[PRESS] {event['event_name']} locked after publication; no repeat.")
        else:
            if bot.DRY_RUN:
                _status(run_exit="dry_run", posted_count=0)
                return 0
            _status(run_exit="post_not_completed", posted_count=0, posting_failures=[{"error_type": "UnconfirmedDelivery"}])
            return 1
        return 0
    finally:
